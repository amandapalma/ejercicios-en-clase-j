...
const ensureData = data => {
  ...
  // es una buena práctica asegurarme de que mis datos son lógicos y coherentes después de recibirlos del servidor,
  // no vaya a ser que el servidor esté fallando o lo hayan cambiado
  // de esta forma dentro de mi web siempre trabajo con datos seguros y fiables
...
const getDataFromServer = function (ev) {
  ...
  fetch('./server-data/perretes.json')
    .then(response => {
      hideLoading();
      console.log('El servidor contesta de forma asíncrona con la respuesta', response);
